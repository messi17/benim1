...
//------------------------------------------------------------------------
function drawPath (cell1, cell2,hue) {

  if (! cell1.vertices) cell1.size();
  if (! cell2.vertices) cell2.size();
  ctx.beginPath();
  ctx.moveTo (cell1.xc, cell1.yc);
  ctx.lineTo (cell2.xc, cell2.yc);
...
*/
  ...
  for (let k = 0; k < 6; ++k) {
    ...
    if (! cell.blocked[side] && !cell.neighbour(side).explored) { // can be explored !
      if (!me) {
        drawPath(cell, cell.neighbour(side),this.hue);
        me = cell.neighbour(side);
      }
      else {
        explorers.push(new Explorer(cell.neighbour(side), (this.hue + intAlea(5,10) % 360)));
        drawPath(cell, cell.neighbour(side),this.hue);
      }
    }
  } // for k
...
//        mouse.x = maxx/2; mouse.y = maxy / 2;
      ...
      case 2 :     // exploring
        ...
        explorers.forEach((expl, k) => {
          if (!expl. explore()) explorers.splice(k, 1); // remove finished explorers
        }) // explorers.forEach
        break;
